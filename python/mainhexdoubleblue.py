...
def main():
    ...
    options = getopt.getopt(sys.argv[1:], 's:g:rdf:a:nq:b:t:p:clm:')
    ...
    if doubleBot:
        ...
        try:
            ...
            init = [0 for i in range(48 + 6*6 + middleSize*9 + middleSize)]
    else:
        try:
            data = file(brainsave,"r")
            init = readString(data)
    ...
    for runs in range(gens):
        ...
        agents = []
        candideWeights = [5.0,0.5]
        balancedWeights = [1.0, 1.0]
        montecristoWeights = [0.5, 5.0]
        weights = [candideWeights, balancedWeights, montecristoWeights]
        for i in range(totalagents):
            if i < numagents:
                if doubleBot:
                    agents.append(NNAgent(DoubleBrain(size, strings[i])))
                elif postBot:
                    agents.append(NNAgent(PostProcessBrain(size, strings[i], middleSize)))
                else:
...
#	    blueloss = (-1 * (blueloss < basePt) + 1 * (blueloss >= basePt)) * (blueloss - basePt)**2 + basePt
            ...
            fits = [losspercent, redloss, greenloss, blueloss, totalDiff, illegalfit]
            cma.strings[i].fitness = fits
        ...
        blueavg = sum(map(lambda x: x[3], halffit)) / halflength * maxloss / 2.0
	diffavg = sum(map(lambda x: x[4], halffit)) / halflength * maxloss
        ...
        if againstBotsOnly or trainIllegal:
            ...
            fitfile.write(str(avg) + ", " + str(redavg) + ", " + str(greenavg) + ", " + str(blueavg) + ", " + str(diffavg) + ", " + str(illfit) + "\n")
            fitfile.close()
        if trainMO:
            level = 0.25
        elif trainIllegal:
        ...
        if againstBotsOnly and changebotfreq:
            ...
            print "Not changing!"
        if trainMO:
            print "Average: %.3f, RedAvg: %.3f, GreenAvg: %.3f, BlueAvg: %.3f, DiffAvg: %.3f, Illfit: %.3f" % (avg, redavg, greenavg, blueavg, diffavg, illfit)
        else:
